...
- Sanity schema consolidated: programTrack, program, programFocus, programPageContent all
  removed/orphaned — replaced by a single programsPageContent singleton document
- programsPageContent schema owns the entire page: hero, offers section header, offer cards
  (inline array), "who it's for" section, and CTA
- Migration script written and run: scripts/migrate-programs-page.mjs populated the
  programsPageContent document in Sanity with all content
- ProgramTrackCard updated: internal links use Next.js <Link>, external links use <a>
  with target="_blank" rel="noopener noreferrer"
- "Who this is built for" section moved from standalone section into PageHero rightColumn
  (3fr/2fr split layout), removing an empty-looking standalone block
...
- Orphaned components removed from barrel exports: FocusAreas, SupplementalLearning,
  ProgramTracks no longer exported from sections/index.ts
- Program and ProgramFocus interfaces removed from lib/types.ts
- Dead query functions removed from lib/sanity.ts: getPrograms, getProgramBySlug,
  getProgramsByCategory, getProgramsFocusItems, getProgramTracks
...
- Offer cards live as inline objects inside programsPageContent (not separate documents)
  — they only belong to this page, so separate documents added no value
...
Pages completed so far:
  - About page — COMPLETE
  - Home page — in progress (hero copy next)
...
Next session:
  - Homepage hero copy
  - Contact page copy
  - Calendly link already live, wire into contact page CTA
...
8-week program structure (confirmed by Jon, adjust as needed):
  Week 1 — Body audit: what you're already communicating without knowing it
  Week 2 — Posture and grounding: how you take up space
  Week 3 — Movement fundamentals: isolation, weight, rhythm
  Week 4 — Stillness and timing: the power of not moving
  Week 5 — Eye contact, breath, and voice as physical tools
  Week 6 — High-stakes situations: presentations, interviews, difficult conversations
  Week 7 — Building your own style: freestyle principles
  Week 8 — Integration: putting it all together, what's next
