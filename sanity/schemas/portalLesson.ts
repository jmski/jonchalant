// Gated portal content. Shown only to authenticated users inside /portal and /lessons/[courseSlug]/[lessonSlug].
import { defineType, defineField } from 'sanity'
import type { StringRule, SlugRule, NumberRule } from 'sanity'
...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'text',
      validation: (Rule: StringRule) => Rule.required().min(50),
    ...
    defineField({
      ...
      type: 'string',
      validation: (Rule: StringRule) => Rule.required().regex(/^[a-zA-Z0-9_-]{11}$/),
    ...
    defineField({
      ...
      type: 'number',
      validation: (Rule: NumberRule) => Rule.required().min(1).max(180),
